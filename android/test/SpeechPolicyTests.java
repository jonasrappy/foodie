...
public final class SpeechPolicyTests {
    ...
    public static void main(String[] args) {
        ...
        check(language.recover(12) == SpeechPolicy.Action.NEXT && language.index() == 1, "unsupported Danish must try the next provider");
        check(language.recover(13) == SpeechPolicy.Action.STOP, "missing Danish must stop, never fall back to English");
        ...
        check(SpeechPolicy.priority("com.google.android.googlequicksearchbox", false, false) < SpeechPolicy.priority("com.samsung.svoice", true, false), "prefer Google for Danish when installed");
        ...
        check(!SpeechPolicy.message(12).contains("forbindelse") && !SpeechPolicy.message(9).contains("forbindelse"), "language/permission errors are not network failures");
        System.out.println("PASS: provider fallback, bounded reconnects, Danish-only language handling, permission/silence handling and precise errors");
    }
}
