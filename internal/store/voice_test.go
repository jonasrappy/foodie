...
func TestVoiceAdditionAndDuplicateDialog(t *testing.T) {
	...
	reply, state, err := s.ApplyVoice(ctx, id, id, cmd("vindruer", 3, "stk."), nil)
	...
	_, state, err = s.ApplyVoice(ctx, id, id, nil, nil)
	if err != nil || state.Revision != original {
		t.Fatal("replay mutated", err)
	}
	id = "voice-duplicate-000001"
	reply, state, err = s.ApplyVoice(ctx, id, id, cmd("VINDRUER", 1, "stk."), nil)
	...
	yes := true
	reply, state, err = s.ApplyVoice(ctx, id, "voice-answer-00000001", nil, &yes)
	if err != nil || reply.Kind != "added" || len(state.Shopping) != 1 || state.Shopping[0].Quantity != 4 {
		t.Fatal(reply, state, err)
	}
	_, again, err := s.ApplyVoice(ctx, id, "voice-answer-00000001", nil, &yes)
	if err != nil || again.Revision != state.Revision || again.Shopping[0].Quantity != 4 {
		t.Fatal("duplicate answer", err)
	}
	id = "voice-decline-0000001"
	_, _, _ = s.ApplyVoice(ctx, id, id, cmd("vindruer", 1, "stk."), nil)
	no := false
	reply, state, err = s.ApplyVoice(ctx, id, "voice-no-answer-00001", nil, &no)
...
func TestVoiceConfirmationMustMatchCurrentRowsAndAnswer(t *testing.T) {
	s := testStore(t)
	ctx := t.Context()
	_, state, _ := s.ApplyVoice(ctx, "voice-seed-00000001", "voice-seed-00000001", cmd("vindruer", 3, "stk."), nil)
	...
	reply, state, err := s.ApplyVoice(ctx, id, answer, nil, &yes)
	...
	replay, state, err := s.ApplyVoice(ctx, id, answer, nil, &yes)
	if err != nil || replay != reply || state.Shopping[0].Quantity != 8 {
		t.Fatal("stale yes replay applied", replay, state, err)
	}
	reply, state, err = s.ApplyVoice(ctx, id, "voice-fresh-answer-01", nil, &yes)
...
func TestVoiceCheckedUnitsAndReset(t *testing.T) {
	...
	reply, _, err := s.ApplyVoice(ctx, id, id, cmd("vindruer", 1, "stk."), nil)
	if err != nil || !strings.Contains(reply.Speech, "krydset af") {
		t.Fatal(reply, err)
	}
	yes := true
	_, state, err = s.ApplyVoice(ctx, id, "voice-checked-yes-01", nil, &yes)
	if err != nil || len(state.Shopping) != 2 || !state.Shopping[0].Checked || state.Shopping[1].Checked {
		t.Fatal(state, err)
	}
	id = "voice-mixed-units-01"
	reply, _, err = s.ApplyVoice(ctx, id, id, cmd("vindruer", 2, "bakker"), nil)
	...
	reply, state, err = s.ApplyVoice(ctx, id, "voice-reset-answer1", nil, &yes)
...
func TestConcurrentVoiceAndDurableTranscript(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			defer wait.Done()
			if _, _, err := s.ApplyVoice(ctx, "voice-race-000000001", "voice-race-000000001", cmd("mælk", 2, "liter"), nil); err != nil {
				t.Error(err)
			}
		}()
	}
...
func TestVoiceFractionalIncrement(t *testing.T) {
	s := testStore(t)
	_, _, err := s.ApplyVoice(t.Context(), "fraction-seed-000001", "fraction-seed-000001", cmd("mælk", .1, "liter"), nil)
	if err != nil {
		t.Fatal(err)
	}
	reply, _, err := s.ApplyVoice(t.Context(), "fraction-next-000001", "fraction-next-000001", cmd("mælk", .2, "liter"), nil)
	if err != nil || !strings.Contains(reply.Speech, "bliver 0,3 liter") {
		t.Fatal(reply, err)
	}
	yes := true
	_, state, err := s.ApplyVoice(t.Context(), "fraction-next-000001", "fraction-answer-001", nil, &yes)
